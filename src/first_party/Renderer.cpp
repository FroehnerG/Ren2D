...
void Renderer::Render(vector<Actor>* actors, int& x_resolution, int& y_resolution)
{
    ...
    for (const auto& actor : *actors) {
        ...
        // Adjust pivot relative to the new scaled size
        SDL_FPoint pivot = {
            actor.view_pivot_offset.x * dstrect.w / img_width,
            actor.view_pivot_offset.y * dstrect.h / img_height
        };

        // Adjust position based on the pivot
        dstrect.x -= pivot.x;
        dstrect.y -= pivot.y;
