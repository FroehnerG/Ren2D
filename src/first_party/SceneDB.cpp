...
void SceneDB::LoadActors(rapidjson::Document& scene_json, SDL_Renderer* renderer, ImageDB* imageDB, AudioDB* audioDB, EventListener* eventListener)
{
	...
	for (auto& actor_json : scene_json["actors"].GetArray()) {
		...
		if (actor_json.HasMember("template")) {
			...
			LoadTemplate(template_name, renderer, imageDB, audioDB, eventListener, current_actor_id);

			actor = templateDB.UseTemplate(template_name);
		}

		actor.ParseActorFromJson(renderer, imageDB, audioDB, eventListener, actor_json, current_actor_id);
...
void SceneDB::LoadTemplate(string template_name, SDL_Renderer* renderer, ImageDB* imageDB, AudioDB* audioDB, EventListener* eventListener, int current_actor_id)
{
	TemplateDB& templateDB = TemplateDB::GetInstance();
	...
	templateDB.LoadTemplate(template_name, renderer, imageDB, audioDB, eventListener, template_json, current_actor_id);
}
